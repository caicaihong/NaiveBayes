最为广泛的两种分类模型是 [决策树模型(Decision Tree Model)](http://www.cnblogs.com/TTyb/p/6279023.html) 和 [朴素贝叶斯模型（Naive Bayesian Model，NBM）]()。

### 朴素贝叶斯算法思路

朴素贝叶斯法是基于 `贝叶斯定理与特征条件独立假设` 的分类方法，按照以前 [决策树](http://www.cnblogs.com/TTyb/p/6279023.html) 的数据，利用朴素贝叶斯进行分类：
